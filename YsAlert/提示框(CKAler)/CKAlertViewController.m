...
- (void)viewDidLayoutSubviews {
    ...
    self.view.backgroundColor = [[UIColor darkGrayColor]colorWithAlphaComponent:.4];
...
#pragma mark 按钮根据功能关键字 颜色设置
        ...
        if ([RedKeys containsObject:btn.titleLabel.text]) {
            [btn setTitleColor:[UIColor colorWithRed:200.0/255 green:0 blue:0 alpha:1] forState:UIControlStateNormal];
        }

        NSArray *LightKeys=@[@"取消", @"放弃", @"稍后", @"以后", @"再想想"];
        if ([LightKeys containsObject:btn.titleLabel.text]) {
...
//        separatorLine.backgroundColor = [UIColor colorWithWhite:0.84 alpha:1];
...
- (void)updateAllButtonsFrame {
    ...
    for (int i=0; i<self.actions.count; i++) {
        ...
        //纵线
        UIView *valy=[[UIView alloc]initWithFrame:CGRectMake(0, 0, btn.frame.size.width, 1)];
        valy.backgroundColor = [UIColor colorWithWhite:0.94 alpha:1];
        [btn addSubview:valy];
    }
    ...
    if (self.actions.count==2) {
        ...
        CGFloat buttonX = self.actions.count>2 ? 0 : buttonWidth;
        CGFloat buttonY = self.actions.count>2 ? firstButtonY+_buttonHeight : firstButtonY;
        UIView *valy=[[UIView alloc]initWithFrame:CGRectMake(buttonX, buttonY+ 10, 1, _buttonHeight-20)];
        valy.backgroundColor = [UIColor colorWithWhite:0.94 alpha:1];
        [_contentView addSubview:valy];
    }
}
...
- (void)showAppearAnimation {
    ...
    if (_firstDisplay) {
        ...
        [UIView animateWithDuration:0.4 delay:0 usingSpringWithDamping:0.45 initialSpringVelocity:10 options:UIViewAnimationOptionCurveEaseIn animations:^{
